...
The two Windows executables are universal x86/x64/ARM64 hosts. `SNAPVERE.apk` is the installable Android CI/debug-signed package built from the same validated 0.1.0 Android source that also passes release-variant lint/build checks. It does not require a private production keystore and must not be represented as a Google Play/production-signed APK. The Android source ZIP is generated directly from the validated Git tree, not from a working directory containing build output.

Because the public APK uses the CI/debug signing identity, users should not rely on it as a stable production-signing lineage for future in-place upgrades. A later production-signed Android channel may require uninstall/reinstall if its signing identity differs.

The release workflow fails before tag creation or publication when any Windows/Android validation gate fails, when the APK signature/alignment checks fail, or when the release directory does not contain exactly the four approved assets.
...
- real rendered WinUI QA in the normal CI path for the release commit;
...
- Android `lintDebug`, `lintRelease`, JVM unit tests, debug build and release build;
- APK signature and ZIP-alignment verification;
