...
SNAPVERE 0.1.14 makes recording control smaller and clearer: Settings has separate compact Start and Stop controls, while active recording uses a small borderless always-on-top timer with a dedicated Stop action. Setup now guides installation through license acceptance and installation options as separate steps, and successful install/uninstall completion correctly leaves the busy state before Finish. Region and Screen Capture continue to follow the display under the pointer, while recording remains primary-display H.264 MP4 and video-only; microphone and system audio are not claimed as supported.
