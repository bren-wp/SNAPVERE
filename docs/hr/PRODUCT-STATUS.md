# SNAPVERE 0.1.14 Status proizvoda
...
SNAPVERE 0.1.14 zadržava local-first snimanje primarnog Windows zaslona preko Windows.Graphics.Capture uz H.264 MP4 enkodiranje i čini stanje upravljanja jasnim: Postavke imaju odvojene kompaktne Start i Stop akcije, a aktivno snimanje koristi mali borderless kontroler s proteklim vremenom i namjenskim Stop gumbom. Setup sada odvaja prihvat licence od instalacijskih opcija i nakon uspješnog install/uninstall postupka izlazi iz busy lifecyclea prije gumba Završi. Responsive high-DPI sizing ostaje zadržan kroz Tray, Settings, Language, About, capture feedback i Setup. Početni način snimanja je samo video; sistemski zvuk i mikrofon nisu navedeni kao podržani.
...
Aktualni 0.1.14 ugovor sadrži `SNAPVERE-Setup.exe`, `SNAPVERE-Portable.exe`, `SNAPVERE-Chrome.zip`, `SNAPVERE-Edge.zip`, `SNAPVERE-Opera.zip` i `SNAPVERE-Firefox.zip`.
...
Javno izdanje je **v0.1.14**. Kasniji neobjavljeni hardening na `main` grani predstavlja izvorni kod dok se buduća verzija izričito ne zapakira i objavi.

Povezano: [Performanse i stabilnost](PERFORMANCE.md), [QA matrica](QA-MATRIX.md) i https://github.com/bren-wp/SNAPVERE/releases/tag/v0.1.14
