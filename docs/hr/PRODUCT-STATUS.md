# SNAPVERE 0.1.7 Status proizvoda
...
Najnoviji capture-path hardening uklanja redundantne full-frame staging alokacije iz Region i Window prikaza te oslobađa raw frozen monitor buffere nakon što je odgovarajući UI bitmap spreman. SNAPVERE 0.1.7 dodatno učvršćuje Setup protiv reparse-point/symlink putanja i marker-prefix spoofinga, sanitizira deferred installer greške te poboljšava tipkovnicu, DPI i narrow-screen ponašanje instalera. SNAPVERE 0.1.7 dodatno objavljuje PNG datoteke uz collision-safe dodjelu naziva na commit granici, kontrolira očekivane Settings/Recent shell i lokalne security greške, vraća jezični odabir na stvarno spremljenu vrijednost nakon neuspjelog zapisa te drži tray fallback unutar Windows virtualnog desktopa.

## Browseri

Chrome, Edge, Opera i Firefox održavaju visible-area, selected-region i bounded full-page capture. SNAPVERE brand je zaključan. Validirani permission contract je `activeTab`, `scripting`, `downloads`, `downloads.open` i `storage`; `downloads.open` koristi se samo za izričitu Recent > Otvori radnju. Široki host pristup nije dio održavanog dizajna. SNAPVERE 0.1.7 dodatno provjerava sender i active-tab ownership prije privilegiranih capture/download radnji. SNAPVERE 0.1.7 dodatno učvršćuje capture-lock ownership, async redoslijed Recent rezultata, zaštitu od dvostrukih akcija, kompatibilnost browser API poziva i responsive/reduced-motion ponašanje te premješta macOS Full Page prečac sa sistemski rezervirane kombinacije Command+Shift+3.

## Paketi

Aktualni 0.1.7 ugovor sadrži `SNAPVERE-Setup.exe`, `SNAPVERE-Portable.exe`, `SNAPVERE-Chrome.zip`, `SNAPVERE-Edge.zip`, `SNAPVERE-Opera.zip` i `SNAPVERE-Firefox.zip`.
...
Javno izdanje je **v0.1.7**. Kasniji neobjavljeni hardening na `main` grani predstavlja izvorni kod dok se buduća verzija izričito ne zapakira i objavi.

Povezano: [Performanse i stabilnost](PERFORMANCE.md), [QA matrica](QA-MATRIX.md) i https://github.com/bren-wp/SNAPVERE/releases/tag/v0.1.7
