# Podrška i pravni linkovi u prozoru O programu

SNAPVERE prozor **O programu** prikazuje službene kontaktne i pravne odredišne adrese bez uvođenja ikakvog mrežnog rada tijekom normalnog pokretanja aplikacije ili mirovanja u trayu.

## Odredišta

- Službena stranica proizvoda: `https://snapvere.com`
- E-mail podrške: `info@snapvere.com` putem `mailto:` veze
- Privatnost: `https://snapvere.com/privacy`
- Uvjeti: `https://snapvere.com/terms`
- Razvojni/publisherski web: `https://brendigo.com`

Sve veze pokreće isključivo korisnik klikom. SNAPVERE ne kontaktira ta odredišta u pozadini i pri otvaranju prozora O programu ne šalje sadržaj snimki zaslona, postavke, telemetriju niti identifikatore.

Putanje `/privacy` i `/terms` definirane su kao stabilne službene putanje proizvoda kako instalirane verzije aplikacije ne bi trebalo mijenjati pri kasnijim izmjenama sadržaja web stranice.
