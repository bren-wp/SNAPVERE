...
SNAPVERE 0.1.14 retains local-first primary-display screen recording through Windows.Graphics.Capture with H.264 MP4 encoding and makes control state explicit: Settings exposes separate compact Start and Stop actions and active recording uses a compact borderless elapsed-time controller with a dedicated Stop action. Setup now separates license acceptance from installation options and clears its busy lifecycle before Finish after successful install or uninstall. Responsive high-DPI sizing remains in place across Tray, Settings, Language, About, capture feedback and Setup. The initial recording mode is video-only; system audio and microphone capture are not claimed as supported.
