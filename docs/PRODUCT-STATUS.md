# SNAPVERE 0.1.20 Product Status
...
GitHub release ZIPs are not represented as externally approved store listings unless that publication has actually happened. SNAPVERE 0.1.20 also tightens browser message sender and active-tab ownership checks before privileged capture/download work.

## Packaging

The active 0.1.20 package contract contains:
...
The public release is **v0.1.20**. Later `main` hardening remains source state only until a future version is explicitly packaged and published.

See [Performance & Stability](PERFORMANCE.md), [QA Matrix](QA-MATRIX.md) and the [current release](https://github.com/bren-wp/SNAPVERE/releases/tag/v0.1.20).
