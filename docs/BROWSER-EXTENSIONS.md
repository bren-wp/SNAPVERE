...
Full-page capture uses explicit limits for tile count, canvas dimensions and total pixels. Tiles are decoded, drawn into one bounded destination canvas and released immediately instead of being retained as a second full image set. Capture-session tokens, tab ownership checks and cleanup watchdogs prevent stale work from silently completing against the wrong tab or browser window. Ownership is revalidated against the initiating tab/window IDs and the last-focused browser window before Region/Full Page script injection and before page-mutating preparation, scrolling, floating-element hiding and assembly boundaries; a Region overlay created just before an ownership change is removed through a token-scoped cleanup request.
